	...
	*/
	...
	public:
		...
		*/
		...
		static constexpr double frictionCoefficient = 0.99;
			...
			*/
			for (const Polygon& poly : app->state.shapes) {
				for (int idx = 0; idx < poly.numbSegments(); idx++) {
					...
					/* DERIVED FROM PSUEDO CODE GENERATED BY AN LLM  */
					...
					// apply collision impulse as instant velocity change
					double impulse = -(1 + ball.elasticity) * relativeVelocity * ball.mass;
					olc::vd2d impulseVector = impulse * normalizedCollisionNormal;
					ball.velocity += impulseVector / ball.mass;
					// apply velocity change from friction impulse
					/*
						when the dot product between two vectors is zero, they are perpendicular. 
						We can use this fact to derive a perpendicular vector to the collision normal from 
						itself.
					*/
					olc::vd2d collisionTangent = olc::vd2d(- normalizedCollisionNormal.y, normalizedCollisionNormal.x).norm();
					double tangentSurfaceSpinVelocity = ball.angularVelocity * ball.radius;
					double surfaceTangentVelocity = ball.velocity.dot(collisionTangent);
					double slip = surfaceTangentVelocity - tangentSurfaceSpinVelocity;

					double maxFrictionImpulse = ball.frictionCoefficient * impulse;
					double frictionImpulseMagnitude = std::clamp(-slip * ball.mass, -maxFrictionImpulse, maxFrictionImpulse);

					olc::vd2d frictionImpulseVector = collisionTangent * frictionImpulseMagnitude;
					ball.velocity += frictionImpulseVector / ball.mass;
					
					olc::vd2d wrench = -normalizedCollisionNormal * ball.radius;
					double torque = wrench.cross(frictionImpulseVector);
					ball.angularVelocity += torque / ball.inertiaMoment;
					
					/* END OF CODE DERIVED FROM PSEUDO CODE GENERATED BY A LLM */
