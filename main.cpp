			...
			for (const Polygon& poly : app->state.shapes) {
				for (int idx = 0; idx < poly.numbSegments(); idx++) {
					...
					*/
					...
					double relativeTangentVelocity = ball.velocity.dot(collisionTangent);
					double frictionImpulse = -std::min(ball.frictionCoefficient * std::abs(impulse), ball.mass * std::abs(relativeTangentVelocity));
					olc::vd2d frictionImpulseVector = 
						(relativeTangentVelocity >= 0 ? 1.0 : -1.0) * collisionTangent * frictionImpulse;
					ball.velocity += frictionImpulseVector / ball.mass;
					
					// TODO use the ball's angular velocity to update the linear velocity
					// TODO use friction to update the ball's angular velocity
